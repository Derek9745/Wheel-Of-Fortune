...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self,category, obscuredPhrase,guessed):
        print(f"{self.name} has ${self.prizeMoney}/n Category: {category}/n Phrase:  {obscuredPhrase}/nGuessed: {guessed}") 
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def __init__(self,name, difficulty):
        super(WOFComputerPlayer,self).__init__(name)
    ...
    def getMove(self,category, obscuredPhrase,guessed):
        #self.SORTED_FREQUENCIES = [self.SORTED_FREQUENCIES]
        if self.getPossibleLetters(guessed) == []:
            return "pass"
        else:
            if self.smartCoinFlip() == True:
                indexCount = 0
                for possibleLetter in self.getPossibleLetters(guessed):
                    if self.SORTED_FREQUENCIES.index(possibleLetter) >= indexCount:
                        indexCount  = self.SORTED_FREQUENCIES.index(possibleLetter)
                return self.SORTED_FREQUENCIES[indexCount]
            else:
